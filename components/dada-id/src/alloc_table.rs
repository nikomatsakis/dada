...
pub struct AllocTable<K: FromId, V> {
    vec: IndexVec<salsa::Id, V>,
    phantom: PhantomData<K>,
}

impl<K: FromId, V> Default for AllocTable<K, V> {
    fn default() -> Self {
        Self {
...
impl<K: FromId, V> AllocTable<K, V> {
...
impl<K: FromId, V> std::ops::Index<K> for AllocTable<K, V> {
    type Output = V;
...
impl<K: FromId, V> std::ops::IndexMut<K> for AllocTable<K, V> {
